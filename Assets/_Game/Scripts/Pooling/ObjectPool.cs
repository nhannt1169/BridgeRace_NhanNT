...
public class ObjectPool
{
    ...
    public static void CreatePool(GameUnit prefab, int amount, Utils.PoolType poolType)
    {
        if (!pools.ContainsKey(poolType) || pools[poolType] == null)
        {
            Pool pool = new();
            pool.CreatePool(prefab, amount);
            pools[poolType] = pool;
        }
    }
...
public class Pool
{
    ...
    public void CreatePool(GameUnit prefab, int amount)
    {
        this.prefab = prefab;
        for (int i = 0; i < amount; i++)
        {
            DespawnObject(SpawnObject(prefab.TF.position, Quaternion.identity, true, null));
        }
    }
    ...
    public GameUnit SpawnObject(Vector3 pos, Quaternion rot, bool forceCreate, Transform parent)
    {
        ...
        {
            unit = inactives[inactives.Count - 1];
